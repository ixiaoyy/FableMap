...
assert.equal(view.signals.find((signal) => signal.id === 'feedback').value, '店主可见')
...
assert(componentSource.includes('不做访客好友、私信、公开访客墙或全局社交图谱'))
...
assert(tavernRouteSource.includes('TavernActivitySignalsCard'))
assert(tavernRouteSource.includes('<TavernActivitySignalsCard tavern={tavern} />'))
assert(tavernRouteSource.includes('NeighborhoodRumorBubble'), 'route should keep ambient rumor surface')
assert(tavernRouteSource.includes('这不是公开留言墙'), 'route should keep owner-visible feedback boundary')
...
assert(combined.includes('无全局社交图谱') || combined.includes('或全局社交图谱'), 'global social graph may only appear as a negated boundary')
