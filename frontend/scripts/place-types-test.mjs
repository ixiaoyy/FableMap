...
const hospitalDisplay = derivePlaceTypeDisplay("hospital")
assert.equal(hospitalDisplay.label, "医院")
assert.equal(isDiscoverablePlaceType("hospital"), true, "Hospital 应进入公开发现筛选")
assert.equal(DISCOVERABLE_PLACE_TYPES.some((type) => type.id === "hospital"), true, "公开筛选 chips 包含 Hospital")
...
assert.equal(placeTypeMatchesTavern(hospitalTavern, "hospital"), true)
