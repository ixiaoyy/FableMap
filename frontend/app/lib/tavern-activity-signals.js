export const TAVERN_ACTIVITY_GUARDRAILS = [
  '无访客好友 / 私信',
  '无公开访客墙',
  '无全局社交图谱',
  '反馈只给店主治理',
...
export const TAVERN_ACTIVITY_FORBIDDEN_COPY = [
  '好友',
  '私信',
  '公开访客墙',
  '全局社交图谱',
...
function hasLocalRumorPack(tavern = {}) {
  return (Array.isArray(tavern.skill_packs) ? tavern.skill_packs : []).some((pack) => {
...
export function buildTavernActivitySignals(tavern = {}) {
  const visitCount = safeNumber(tavern.visit_count)
  const characterCount = countItems(tavern.characters)
  const gameplayCount = countItems(tavern.gameplay_definitions)
  const hasRumor = hasLocalRumorPack(tavern)
