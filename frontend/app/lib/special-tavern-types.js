...
function hasEnabledSkillPack(tavern, expectedPackIds = []) {
  const activeIds = new Set(expectedPackIds.map((id) => normalizeText(id)))
  if (!activeIds.size || !Array.isArray(tavern?.skill_packs)) return false
  return tavern.skill_packs.some((pack) => {
...
function buildTavernSearchText(tavern = {}) {
  return [
    ...
    tavern.scene_prompt,
    tavern.address,
    ...worldInfoSearchText(tavern.world_info),
    ...(Array.isArray(tavern.characters)
...
export const SPECIAL_TAVERN_TYPES = [
  {
    ...
    description: "薄层只负责识别、展示和模板初始化；数字人档案仍需用户确认，可适配 FableMap / SillyTavern，也可复制到视频或短剧工具。",
...
const SPECIAL_TAVERN_TYPE_BY_ID = new Map(SPECIAL_TAVERN_TYPES.map((type) => [type.id, type]))

export function normalizeSpecialTavernTypeId(value) {
  const normalized = normalizeText(value).replace(/_/g, "-")
  return SPECIAL_TAVERN_TYPE_BY_ID.has(normalized) ? normalized : ""
}

export function buildSpecialTavernTypeDraftSeed(value) {
  const specialType = SPECIAL_TAVERN_TYPE_BY_ID.get(normalizeSpecialTavernTypeId(value))
  return specialType ? { ...specialType.draftSeed } : null
}

export function deriveSpecialTavernType(tavern = {}) {
  if (typeof tavern === "string") {
    const id = normalizeSpecialTavernTypeId(tavern)
    return id || null
  }

  const layoutStyle = toText(tavern.layout_style).toLowerCase() || "lobby"
  const text = buildTavernSearchText(tavern)

  for (const specialType of SPECIAL_TAVERN_TYPES) {
    if (hasEnabledSkillPack(tavern, specialType.skillPackIds)) return specialType.id
...
export function specialTavernTypeMatchesTavern(tavern, specialTavernTypeId) {
  const expected = normalizeSpecialTavernTypeId(specialTavernTypeId)
  if (!expected) return false
  return deriveSpecialTavernType(tavern) === expected
}

export function deriveSpecialTavernTypeDisplay(tavernOrType = {}) {
  const id = typeof tavernOrType === "string"
    ? normalizeSpecialTavernTypeId(tavernOrType)
    : deriveSpecialTavernType(tavernOrType)
  return id ? SPECIAL_TAVERN_TYPE_BY_ID.get(id) || null : null
}
