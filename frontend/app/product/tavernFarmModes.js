...
export function getFarmProgressStorageKey(tavernId = 'tavern', visitorId = 'visitor') {
  return `fablemap_farm_progress:${tavernId || 'tavern'}:${visitorId || 'visitor'}`
}

export function loadFarmProgress(tavernId, visitorId) {
  if (typeof window === 'undefined' || !window.localStorage) {
    return normalizeFarmProgress()
  }
  try {
    const raw = window.localStorage.getItem(getFarmProgressStorageKey(tavernId, visitorId))
...
export function saveFarmProgress(tavernId, visitorId, progress) {
  const normalized = normalizeFarmProgress(progress)
  if (typeof window !== 'undefined' && window.localStorage) {
    try {
      window.localStorage.setItem(
        getFarmProgressStorageKey(tavernId, visitorId),
...
// ============================================================
// Garden Tavern Currency Economy
// ============================================================
...
// --- In-tavern Ranking (local data only, no backend) ---
