...
import { type Tavern, type TavernCharacter } from "../lib/taverns"
import { cn } from "../lib/utils"
import { Button } from "../ui/button"

type TavernPreviewModalProps = {
  tavern: Tavern | null
  onClose: () => void
}
...
function worldInfoPreview(tavern: Tavern) {
...
export function TavernPreviewModal({ tavern, onClose }: TavernPreviewModalProps) {
  if (!tavern) return null

  const characters = Array.isArray(tavern.characters) ? tavern.characters : []
  const worldInfoEntries = worldInfoPreview(tavern)
  const scenePrompt = tavern.scene_prompt || "店主还没有写下场景提示。"
...
type TavernPreviewCardProps = {
  tavern: Tavern
  onPreview: (tavern: Tavern) => void
  className?: string
}

export function TavernPreviewCard({ tavern, onPreview, className }: TavernPreviewCardProps) {
  const characters = Array.isArray(tavern.characters) ? tavern.characters : []
