...
- `frontend/app/lib/taverns.ts` episode export types and `exportEpisode(...)`
...
Do not call `/api/v1/taverns/.../episodes/export` directly in components.
...
| User downloads/copies markdown | uses `episode.markdown`; does not mutate tavern state |
| Future component | no raw fetch and no all-visitors default |

## Good/Base/Bad Cases

- Good: owner selects one chat session row and calls `exportEpisode({ tavernId, visitorId, characterId })`.
...
- endpoint path is `/api/v1/taverns/{id}/episodes/export`;
- method is `POST`;
- request body preserves `visitor_id`, `character_id`, `title`, and `include_pending`;
- native `frontend/app/lib/taverns.ts` exports `EpisodeExportResponse` and `exportEpisode`.
...
```jsx
await fetch(`/api/v1/taverns/${id}/episodes/export`, { method: 'POST' })
```

### Correct

```jsx
const result = await service.exportEpisode({ tavernId, visitorId, characterId }, userId)
