...
> Logging conventions and sensitive-data boundaries for FableMap backend code.
...
Seen in modules such as `backend/src/fablemap_api/core/web/service.py`, `backend/src/fablemap_api/core/llm_clients.py`, `backend/src/fablemap_api/core/char_card_parser.py`, and `backend/src/fablemap_api/core/world_info_injector.py`.
...
- `api_key`, `Authorization` headers, owner LLM credentials, or tavern keyvault contents.
- Passwords or `password_hash` values.
- Full visitor chat transcripts, private memories, or raw prompts by default.
- Full tavern package imports if they may contain private owner content.
...
```python
    ...
    logger.warning("LLM error for tavern %s: %s", tavern_id, e)
...
1. `backend/src/fablemap_api/core/web/service.py`: logs LLM/output-rule failures while returning user-facing degraded results.
2. `backend/src/fablemap_api/core/llm_clients.py`: module-level logger lives beside backend adapters; provider-specific errors become `LLMError`.
3. `backend/src/fablemap_api/core/char_card_parser.py`: parser logger exists for import/export diagnostics; binary/card parsing should not leak unrelated file content.
4. `backend/src/fablemap_api/core/world_info_injector.py`: injection logic has a module logger; logs should describe matching/injection problems without dumping private conversation history.

---

## Common mistakes

- Using `print()` for backend diagnostics instead of logger methods. CLI startup output in `backend/src/fablemap_api/core/api.py` is an exception because it intentionally prints server metadata.
