...
- System / public-welfare shop owners may later save the managed free-model choice `deepseek-v4-flash-free` via the OpenCode Zen OpenAI-compatible route; that choice must not force the shop closed. During early testing, this opt-in model and legacy managed free markers (`kilo-auto/free`, `glm-4.7-flash`) may be hydrated from the versioned repo config at `backend/config/system_public_welfare_llm.json`; ordinary user taverns must not inherit that test key. The config must read provider credentials from an environment variable such as `OPENCODE_API_KEY`, not from a committed secret. If no matching versioned config or env key exists, runtime keeps the local rules fallback until a configured provider/API endpoint exists.
