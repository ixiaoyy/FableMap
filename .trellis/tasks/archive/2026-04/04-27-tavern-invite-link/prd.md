...
C 阶段让店主看到经营反馈，A 阶段让探索者进入后看到回访关系。B 阶段需要一个最小传播闭环：让任意公开可见的空间详情页能被复制为邀请链接，便于店主或访客把「这间真实坐标锚定的空间」分享出去。

## 目标

- 在空间详情页展示一个轻量「邀请链接」卡片。
- 分享内容只来自当前公开 Tavern payload（名称、描述、坐标、链接），不调用 LLM，不生成/写回空间内容。
