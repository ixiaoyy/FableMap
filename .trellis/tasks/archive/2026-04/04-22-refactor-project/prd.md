...
补充说明：P1.4 前端视觉不应只有一种空间风格。新增 `.trellis/tasks/04-22-refactor-project/prototypes/style-directions/`，把视觉从单一“暖木质空间”扩展为 theme skin 体系。

方向板包括：

* `style-directions/01-warm-cyber-tavern.svg` — 默认暖木质空间。
* `style-directions/02-neon-megacity-cyberpunk.svg` — 霓虹夜城朋克；可参考硬核都市情绪，但不复刻 Cyberpunk 2077 的具体 UI/IP。
...
* `prototypes/01-discover-map.svg` — 发现空间：真实地图锚点、附近空间列表、开店入口。
* `prototypes/02-tavern-entry.svg` — 空间入口：店主设定、访问规则、NPC 选择、入场承诺。
...
用户提出：针对不同风格，设计不同的 NPC 形象放入空间内。

本次按 P1.4 frontend feature extraction 的表现层范围处理：

* 新增原型 `.trellis/tasks/04-22-refactor-project/prototypes/07-npc-style-cast.svg`，记录暖木质空间、霓虹夜城、小清新日式漫画、手绘幻想小镇四类 NPC 形象占位与空间室内投放方式。
* 新增 `frontend/app/features/tavern-npc-stage/`，在 `/tavern/:id` 中渲染空间内 NPC 形象舞台。
...
用户确认：后续 NPC 作图必须符合空间主题，并且必须是实际二次元/游戏动漫人物形象，不接受圆圈方块占位。

本次把该要求落成项目规范与真实前端资产：

* 新增 `.trellis/spec/frontend/npc-art-guidelines.md`，明确 NPC 作图与 fallback 合同：
  * 必须是真实卡通/二次元/游戏动漫风人像。
  * 必须可见空间主题：吧台、木架、酒杯、灯牌、菜单、烛光、地图桌、空间室内氛围等。
  * 不允许几何占位、抽象头像、空背景泛二次元头像、具体 IP 或在世艺术家风格模仿。
  * owner 上传/导入的 `sprites.neutral`、`avatar`、`image_url` 优先；项目 fallback 只用于展示，不写回角色卡。
* 新增项目资产 `frontend/app/assets/npc-style-cast/tavern-npc-style-cast.png`，作为四类空间风格 NPC 真实人像 fallback sheet。
...
* 店主 tavern CRUD、空间包导入导出、语音配置与 TTS 测试也全部切到 native `app/lib/taverns.ts` 客户端。
