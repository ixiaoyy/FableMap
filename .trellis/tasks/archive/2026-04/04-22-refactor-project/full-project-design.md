...
- **Promote**：与空间主链路直接相关、前端仍需要、可测试、无禁区风险。
...
- `/`：产品介绍 + CTA。
- `/discover`：真实地图 / 坐标 / 空间发现。
- `/create`：开店向导。
- `/tavern/:id`：visitor entry + NPC chat + gameplay。
- `/owner`：我的空间 / owner console。
- `/owner/:id`：单间空间配置：角色、世界书、LLM、runtime、voice、package。
...
- 地图是入口，不是产品价值本体。
- visitor 体验优先：看到空间、进门、选 NPC、聊天、记忆反馈。
- owner 体验其次但必须完整：创建、配置角色、配置 LLM、测试、导出/导入。
- 移动窄屏必须可用；新组件不得假设宽屏控制台。
- 视觉风格应支持主题化：同一信息架构可以落在暖木质空间、霓虹夜城朋克、小清新日式漫画、手绘幻想小镇等不同 theme skin 上。
- 主题只能改变表现层 design tokens（色彩、材质、卡片、地图 marker、对话气泡、动效氛围），不得改变 API、Schema、权限、店主主权或主链路语义。
- 可参考外部作品的宽泛情绪，例如“温暖手工空间”“硬核都市”“清新恋爱漫画”“手绘幻想小镇”，但不得复刻具体 IP 的 UI、Logo、字体、角色、专有符号、地图或在世艺术家的个人笔触。

### 8.4 产品原型图产出

P1.4 前端 feature extraction 不能只看目录结构，必须先以可视化原型校准信息架构。当前原型图产出位于 `.trellis/tasks/04-22-refactor-project/prototypes/`。

多风格方向板位于 [`prototypes/style-directions/`](./prototypes/style-directions/)，用于说明 FableMap 支持多种空间皮肤而不是单一视觉：

| 方向板 | 适用氛围 | 实现约束 |
|--------|----------|----------|
| [`01-warm-cyber-tavern.svg`](./prototypes/style-directions/01-warm-cyber-tavern.svg) | 默认暖空间、新手友好、手工铜边 | 不复制具体卡牌/对战语义 |
| [`02-neon-megacity-cyberpunk.svg`](./prototypes/style-directions/02-neon-megacity-cyberpunk.svg) | 雨夜街区、硬核、霓虹招牌 | 不复刻 Cyberpunk 2077 具体 UI/IP |
| [`03-fresh-japanese-romance.svg`](./prototypes/style-directions/03-fresh-japanese-romance.svg) | 小清新、恋爱漫画、治愈空间 | 不复刻具体日漫角色/分镜/IP |
...
| 原型 | 覆盖主链路 | 后续实现入口 |
|------|------------|--------------|
| [`01-discover-map.svg`](./prototypes/01-discover-map.svg) | 真实地图锚点、附近空间列表、开店入口 | `routes/discover.tsx` + `features/tavern-discovery/` |
...
| [`07-npc-style-cast.svg`](./prototypes/07-npc-style-cast.svg) | 多风格 NPC 形象、空间室内舞台、点击切换对话对象 | `features/tavern-npc-stage/` + `routes/tavern.tsx` |
...
- NPC 视觉资产必须遵守 `.trellis/spec/frontend/npc-art-guidelines.md`：作图必须是符合空间主题的真实卡通/二次元/游戏动漫人像，不允许用圆圈、方块、抽象小人或无空间场景的泛头像替代。
