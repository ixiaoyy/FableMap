...
## Requirements
- 首页正文必须由真实 DOM 内容组成：导航、标题、CTA、统计、空间卡片、NPC/记忆说明等都用组件/文本渲染。
- 可继续使用现有空间参考图，但只能作为装饰性 banner、背景或氛围图，不再作为整页截图承载内容。
- 保持 FableMap 主线：真实地点锚定、店主创作空间、AI NPC 对话、记忆回访。
- 保持入口可用：`/discover` 用于进入/探索空间，`/create` 用于开店。
- 窄屏可用：内容应自然堆叠，不依赖 729px 截图比例或绝对定位热点。

## Acceptance Criteria
- [x] `frontend/app/routes/home.tsx` 不再渲染整张首页截图和绝对定位 hotspot 链接。
- [x] 首页包含合理布局：顶部导航、hero、核心指标、热门空间/体验模块、价值条或流程说明。
- [x] 使用拆出的 banner 与模块图片作为 hero、空间卡片、NPC 对话、记忆模块的视觉素材，而不是完整贴图页面。
