...
## Goal
在首页已完成高保真布局后，把原生 React Router 页面 `/discover`、`/create`、`/tavern/:tavernId` 与共享 `ProductShell` 统一到同一套空间视觉语言。
...
- [x] 发现页更像地图发现/空间列表入口，保留现有 loader 与 API 读取逻辑。
- [x] 开店页更像创作者工作台，强调真实坐标、店主创作、访问边界。
- [x] 空间页更像沉浸式入口，保留 NPC 舞台和对话能力。
