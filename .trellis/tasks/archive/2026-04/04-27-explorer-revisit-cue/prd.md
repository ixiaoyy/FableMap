...
FableMap 主链路要求「进入空间 → 对话互动 → 写回记忆 → 回访反馈」。后端 `/api/v1/taverns/{id}/enter` 和 `/chat` 已返回 `visitor_state`，但当前原生空间聊天 UI 只显示首句/降级提示，没有把访客与空间的关系状态展示给探索者。

## 目标

在不新增后端字段、不改变 Schema、不生成空间内容的前提下，让探索者进入空间后能看到一个轻量「回访提示」：
...
1. 未进入空间前，聊天区显示「进入空间后建立回访状态」的低噪提示。
