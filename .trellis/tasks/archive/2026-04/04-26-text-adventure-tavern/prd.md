...
新增一间默认公益空间《午夜委托板》，用现有 `gameplay_definitions` 展示“空间不只是聊天，也可以是结构化轻文字互动”。新空间应在无外部 API Key 的开发环境中可发现、可进入、可对话，并可直接开始多个 published 文游玩法。

## Requirements

- 新增一间默认公益空间《午夜委托板》，挂接真实坐标，并保持 `public` / `open` / `rules` 后端。
- 空间包含 2 个 NPC：委托板值夜员“墨栈”和异常登记员“栀灯”。
- 空间包含文游相关 WorldInfo：委托板、线索调查、社区小委托、异常值班、结算方式、安全边界等。
- 空间包含 3 个 published gameplay：
  - 线索调查：整理线索并给出低风险推理。
  - 社区小委托：把小事拆成可执行行动。
  - 异常值班：处理一段轻都市传说式异常，但不进入恐怖伤害或战斗。
- rules 后端对委托/线索/异常等主题给出本地文游式回复，不消耗 token。
- 补充 focused pytest，覆盖新空间可发现、3 个 published gameplay、fallback 推进、rules 回复。

## Acceptance Criteria

- [x] `DEFAULT_PUBLIC_WELFARE_TAVERN_IDS` 包含 `pw_midnight_commission_board`。
- [x] 查询“文游”或“委托”能发现《午夜委托板》。
- [x] 新空间至少有 2 个角色，且角色字段满足现有 `TavernCharacter` 结构。
- [x] 新空间至少有 3 个 published gameplay，标题分别覆盖线索调查、社区小委托、异常值班。
- [x] 访客可启动并 fallback 推进每个 gameplay。
- [x] 访客可进入新空间并通过 rules 后端获得文游主题回复，不消耗 token。
