...
完成 C/A/B 后，探索者能看到活跃感、回访反馈与邀请链接。D 阶段需要把「我也想开一间」变成低摩擦入口，同时保持主人主权：不能复制别人的空间内容，只能带入坐标/地址这类真实空间锚点。

## 目标

- 在空间详情页提供「在附近开自己的空间」CTA。
- CTA 跳转到 `/create` 并预填当前空间的真实坐标/地址。
- `/create` 接收 query 参数作为默认值，同时明确提示不会复制原空间内容。

## 非目标

- 不复制 tavern name / description / scene_prompt / characters / world_info。
- 不新增后端接口或 schema。
- 不做推荐算法、排行榜、付费升级、平台自动生成空间内容。
...
1. 空间页 CTA 链接包含 lat/lon/address/source_tavern，不包含 name/description/scene_prompt。
2. create 页能从 query 预填坐标/地址。
3. create 页提示「只带入空间锚点，不复制原空间内容」。
