...
## Goal
基于 `设计参考/` 中的空间页参考图，在现有 React Router 空间详情页实现多种可切换的空间体验布局，让页面能覆盖大厅、NPC 会话、任务/玩法、混合房间等场景。
...
- 视觉参考 `设计参考/image.png` 与 `设计参考/企业微信截图_*.png`：沉浸式背景、顶部空间信息、功能卡、NPC 列表/对话、委托/线索/记忆/店内事件模块、底部模式切换。
- 按用户确认，新增 Tavern 级 `layout_style` 字段作为空间页默认布局偏好；不修改 Character schema、不引入新依赖。
- 窄屏可用：布局在移动端堆叠，横向功能项可换行/滚动。

## Acceptance Criteria
- [ ] 空间页顶部出现多布局切换，并能在同一页切换 4 种布局。
...
- 不改范围：Character schema、空间内容生成逻辑、依赖、图片资源目录结构。
