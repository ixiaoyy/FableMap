...
为 FableMap 空间平台实现基于地理位置的时间系统：自动推断时区、精确到分钟的营业时间管理、NPC 状态与对话随时间动态变化，打烊时呈现"熄灯"氛围但不隐藏空间。

## What I already know

* Tavern 模型当前有 `lat`、`lon`、`address`，没有时区和营业时间字段。
* `docs/WORLD_SCHEMA.md` 定义了核心数据模型（v0.5）。
* 用户决策：打烊时空间可搜到但显示"熄灯"效果，精确到分钟考虑季节变化，心情由 AI 综合判断随机生成。

## Product Decisions (from user)

| 决策点 | 选择 |
|-------|------|
| 打烊时空间可见性 | 始终可搜到，显示"熄灯"氛围 |
| 时间粒度 | 精确到分钟，考虑夏令时 |
...
4. **前端展示**
   - 空间卡片/详情显示营业状态（营业中 / 已打烊）
   - 非营业时视觉"熄灯"效果：
     - 整体降低明度（CSS `filter: brightness(0.7)`）
     - 叠加半透明黑色遮罩
     - 图标/装饰暗淡处理
   - 显示空间当地时间（"22:47" 格式）
   - 打烊时空间仍可对话，但 NPC 回复带有"不在营业"氛围
   - **访客时差**：MVP 简单处理，只显示空间本地时间（不显示"你那边是X点"）

5. **API 响应**
   - 空间列表/详情 API 返回 `is_open` 和 `local_time_display`
   - 减少前端重复计算
...
* 允许修改范围：
  ...
  - `frontend/app/components/` (空间状态展示)
