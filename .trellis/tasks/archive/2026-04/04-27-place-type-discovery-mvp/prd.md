...
在不修改后端 Schema、不引入 `Place/Home/Member/Relationship` 持久模型的前提下，让发现页先具备“地点类型”表达与筛选能力：空间仍是默认兼容类型，但用户能看到咖啡店、奶茶店、餐馆、便利店、书店、学校等真实生活地点语义。
...
- Tavern 预览弹窗展示地点类型徽章，帮助用户理解“空间是地点类型之一”。
