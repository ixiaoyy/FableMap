...
4. 默认公益空间 `pw_jingan_catbell_refuge` 的 `char_pw_mimi_nya` 增加项目内头像和多表情 PNG 路径。
...
采用确定性规则生成器：根据空间名称 / 简介 / 标签和店主禁忌方向组合出安全角色卡字段。这样不需要外部 API Key，能稳定测试，并保持“AI 草稿只是创作辅助”的产品边界。
