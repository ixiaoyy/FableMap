...
当前 FableMap 已有默认图片规范和 NPC 完成标准，但部分“产品上明确需要图片”的位置仍可能走默认占位、通用 fallback、低保真图片或缺少表情差分。用户要求：凡是特定需要图片的地方，都应进入美术升级任务范围；后续做新 NPC / 新空间 / 新入口页时，不能只交文字和数据。
...
- 空间详情页 / 入场页 / TavernLayoutShowcase 中明确需要视觉图的位置。
- 空间封面 / 氛围图 / 内景图。
- 公益默认空间如外星便利店、午夜委托板、静安猫铃避难所等应有与主题匹配的高质量视觉资产。
...
- 不新增大型 UI 框架或图片管理依赖。
- 不把 AI 生成图片自动写回用户空间；用户上传 / 导入图片仍优先。
