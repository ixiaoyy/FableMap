# brainstorm: 空间与角色关系联动图谱

## Goal

探索一个“空间↔空间、角色↔角色、跨空间角色↔角色”的关系图谱系统：关系可为友好/亲近/同盟，也可为陌生/竞争/敌对，并影响访客与相关空间或角色的好感/关系变化。

## What I already know

* 用户希望空间之间存在不同关系：亲近、同盟、陌生、敌对等。
* 负向关系：访客与某空间关系越好，可能导致与另一个敌对空间关系越差。
* 正向关系：访客与某空间关系越好，可带动与友好空间关系略微变好，但有上限。
* 角色之间也应有关系；两个空间里的角色之间也可以有独立关系。
...
* 这是产品/架构头脑风暴，不立即实现代码。
* 关系系统应保持 FableMap 主线：地图空间、店主主权、AI NPC 对话、记忆/回访，不变成传统社交网络或战斗阵营系统。
* 关系联动应是可解释、可配置、可测试的数值规则，而不是 AI 自由改写正史。

## Open Questions

* MVP 中是否只做“关系影响好感数值”，还是也要在发现页/空间详情/对话中展示关系提示？

## Requirements (evolving)

* 支持空间与空间之间的关系类型和强度。
* 支持角色与角色之间的关系类型和强度，包含跨空间角色关系。
...
* 不做访客间社交、关注、私信、帮派排行。
* 不做战斗、等级、装备或阵营战争系统。
* 不让平台 AI 自动生成/发布空间关系正史；关系应由店主或明确规则确认。
