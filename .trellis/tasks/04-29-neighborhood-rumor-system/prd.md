...
邻里传闻系统 — NPC 主动分享关于其他空间的传闻，为访客推荐其他空间，增加探索动力。
...
1. **访客在 A 空间聊天时**，NPC 提及 "听说 XX 街新开了家不错的咖啡店"
2. **访客点击传闻链接**，跳转到目标空间
3. **访客在目标空间**可以看到 "来自 [A空间] 的推荐" 标记
...
|------|------|------|
| id | string | 唯一标识 |
| source_tavern_id | string | 消息来源空间 |
| target_tavern_id | string | 推荐目标空间 |
| target_tavern_name | string | 推荐目标空间名称（冗余存储） |
...
**参数**：
- `source_tavern_id` (string, optional): 按来源空间筛选
...
2. **智能推荐**：根据访客兴趣推荐相关空间
3. **来源标记**：在目标空间显示 "来自 [A空间] 的推荐"
4. **传闻管理**：店主可管理自己空间产生的传闻
