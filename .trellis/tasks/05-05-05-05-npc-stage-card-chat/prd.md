...
当前 `/tavern/:id` 的 SillyTavern-style 空间详情页左侧 NPC 列表和中间聊天头部都只使用 48px 小头像。用户确认这会让已制作的 NPC 素材展示价值过低，选择方案 A：在不破坏“进来就聊天”的主链路前提下，把当前选中 NPC 在聊天区顶部舞台化展示。

## Goals

- 保持现有三栏聊天工作台：左侧快速选择 NPC，中间聊天，右侧折叠空间信息。
...
- 不制作或替换新的 NPC 图片资产。
- 不实现独立路由级 NPC 画廊页面；本次只在当前空间工作台内补紧凑展示。
