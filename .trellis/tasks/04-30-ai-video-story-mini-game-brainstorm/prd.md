...
探索是否把“AI 生成剧情视频 + 选择题/复活/短关卡”的小游戏跑量模式，转译成 FableMap 的空间原生能力：让店主为真实坐标空间配置一个可传播、可试玩、可回访的短剧情互动入口，而不是让平台自动生产和发布空间内容。
...
* FableMap 当前主线不是传统小游戏平台，而是“真实地图 → 空间发现 → 进入空间 → 配置 AI NPC → 对话互动 → 写回记忆 → 回访反馈”。
* 项目硬约束：空间必须挂接真实坐标；内容由店主确认；AI 只能生成未发布草稿；不能做平台自动生成并发布的剧情/空间/NPC；不能做战斗、等级、装备、排行榜等传统 RPG 系统。
...
* 这个方向对 FableMap 的价值不在“照抄小游戏”，而在把短剧爽点变成空间的传播入口、访客试玩入口和店主创作模板。
...
|----|----------------|---------------------|
| 视觉包装 | 竖屏短剧/视频画面，看起来像刷短视频 | 空间短剧卡 / 手机竖屏剧情面板；先用背景图、角色头像、场景氛围，不急着接视频生成 |
| 目标表达 | “帮助 X 存活 4 次”这类强目标 | “帮老板撑过 3 位难缠客人”“帮深夜便利店完成 4 次救场” |
| 互动方式 | 2–3 个大按钮选项，低门槛 | 复用 `GameplaySessionPanel` choices，移动端做更强 CTA |
| 情绪节奏 | 夸张冲突、误会、反转、失败提示 | 空间内 NPC/店主确认的轻剧情，避免平台自动编故事 |
| 失败/重玩 | 复活、再来一次、教程感 | FableMap 只做“重新开始/换个选择”；不做广告复活、内购、惩罚数值 |
| 传播钩子 | “这是游戏画面不是视频”反差文案 | “这是你家附近空间里的互动短剧”/“真实地点里的 AI NPC 小剧场” |

FableMap 应借鉴的是 **竖屏短剧情互动表达**，不是复制小游戏商业化系统。

## Open Questions

* MVP 应定位成“店主创作工具”、 “访客发现页传播入口”，还是“空间内玩法体验”？

## Requirements (evolving)

* 必须保持真实地点/空间锚点，不能变成无地图锚点的泛短剧小游戏。
...
* 玩法文案必须表达为空间/地点内的轻剧情体验，不做脱离空间的泛古装/爽文/闯关游戏。
...
目标：把空间发现页卡片改得更像短视频/小游戏广告入口，但点击后仍回到真实坐标空间。

可能交付：

* 对有玩法的空间显示短剧式标题，例如“帮深夜便利店撑过 4 个怪客”。
* 显示一个大 CTA：“进入小剧场”/“开始救场”。
* 空间卡片继续保留真实地点、访问权限、店主确认内容，不变成传统 POI 排行或小游戏列表。

风险：如果没有空间内玩法承接，会变成空包装；适合放在 MVP 1 之后或与 MVP 1 小范围合并。

### MVP 3 — 店主 AI 短剧草稿助手（后续任务）

目标：让店主从空间设定生成“短剧玩法草稿”，但必须审核确认后才能发布。

可能交付：

* 店主输入：空间名、地点类型、NPC、禁忌方向、想要的情绪钩子。
...
* `docs/PRODUCT_BRIEF.md` / `docs/FABLEMAP_TAVERN_PLATFORM.md`: FableMap 是空间 UGC 平台，AI 是 NPC 对话与创作辅助，不替主人自动发布内容。
...
**Approach A: “空间短剧玩法模板” inside existing GameplayDefinition (Recommended)**

* How it works: 给店主提供一组短剧式玩法模板，例如“救场 4 次”“听懂 NPC 暗示”“帮店主化解尴尬”，模板落到现有 `gameplay_definitions.nodes/choices`；访客在空间内用竖屏卡片体验 3–5 步。
* Pros: 最贴合现有能力；不改 schema；风险最低；能立即验证“剧情选择爽点”是否适配空间。
* Cons: 初期缺少真正视频画面，传播冲击力弱于参考小游戏。

**Approach B: “发现页短剧预告卡” as marketing surface**

* How it works: 在 discover/空间卡片上展示一个短剧式封面、冲突标题和 CTA，例如“帮奶茶店老板躲过差评 3 次”；点击后进入对应空间玩法。
* Pros: 更接近跑量传播入口；能提高发现页点击率；仍能保持内容归属空间。
* Cons: 需要更强 UI/视觉设计；如果没有玩法内容承接，会变成噱头。

**Approach C: “店主 AI 短剧草稿助手” for future task**

* How it works: 店主输入空间设定，系统生成未发布的短剧玩法草稿/分镜提示/选项文案，由店主编辑确认后保存。
...
* 与 Create Tavern Step Wizard 有天然联系：创建空间时可选“给新店添加一个 3 步短剧体验”。
...
优先把这个方向收敛成 **Approach A + 少量 Approach B**：先做“空间短剧玩法模板 + 竖屏选择体验”，并允许发现页/空间入口展示短剧式标题与 CTA。AI 视频生成和自动分镜先不进 MVP，后续作为“店主确认的 AI 短剧草稿助手”另开 task 评审。
