...
把 FableMap 首页从 “Tavern-only / 开空间工具” 的表达，升级为 **城市里的生活模拟器**，同时保留原 `index` 设计的视觉强度。第一阶段只做“设计系统 + 首页壳”，不全站铺开。
...
这两张图代表当前不能丢的视觉质量：高密度氛围、霓虹空间、强主视觉、海报/游戏封面感。后续不能退回低保真渐变卡片或解释型 dashboard。
...
首屏副标题也不能列举 Home / 学校 / 店铺 / 空间。对探索者来说，它们先是等价的待探索区域。
...
- 将首页叙事从 “Cyber taverns on real places / 开店 / 空间收录” 调整为 “真实城市里的 AI 生活切片”。
...
- No first-screen type chips. Home / 学校 / 店铺 / 空间 should not be enumerated in the hero; they are all equal explorable areas from the explorer perspective.

Avoid:

- 大段解释性段落。
- 太多指标数字。
- “空间收录 / 店主主权 / API Key” 这类后台或说明性语言占据首屏。
...
- **真实地图是入口**：每个空间有真实坐标。
- **AI 场所会回应你**：Home、学校、店铺、空间都可以承载角色与记忆。
- **关系留在城市里**：角色、成员、地点关系可被回访。

### 3. Featured City Slices

Purpose: 展示多类型 Place，不再只展示空间。
...
Copy should shift from “店主自动生成空间内容边界” to user-facing value:
...
| `空间` nav as primary category | `AI 场所` / `城市切片` |
| `开店` | `创建空间` / `创建我的空间` |
| `空间收录` | `AI 场所` / `城市切片` |
| `热门空间` | `正在发光的地点` / `精选城市切片` |
| `店主入口` | `主人入口` / `管理我的空间` |

`空间` 可以继续作为卡片类型或示例出现，但不再是唯一世界观。
...
3. Confirmed: No Home / 学校 / 店铺 / 空间 type chips on the first screen. Types are equal explorable areas for explorers and should be revealed later.
