...
`/owner` 已经是店主管理入口，并能聚合酒馆、访客状态、会话摘要与部分指标。最近完成的功能（Tavern Visitor Notes、通知 WebSocket、AI Assisted Tavern Drafts、地点类型视觉卡、移动底部导航）还没有在店主视角形成一个清晰的“下一步经营驾驶舱”。

本任务不重新发明 Owner Dashboard，而是在现有 `/owner` route 与 `owner-summary` 边界上做表现化与入口整合，让店主一眼知道：我有哪些酒馆正在营业 / 歇业；最近是否有访客回访、留言或会话；AI/LLM 配置是否阻塞开门；下一步应该去创建、配置、复盘或处理反馈。

## 目标

1. 强化 `/owner` 为高质量赛博酒馆店主驾驶舱，而不是管理表格 MVP。
...
- 不让 AI 自动发布酒馆/NPC 内容；AI 草稿入口只能引导到店主确认流程。
...
作为新店主，我进入 `/owner` 后，如果还没有酒馆，应看到明确的第一步：创建真实坐标锚定的酒馆，并知道 AI 草稿只是辅助创作。

### 店主：已有酒馆但未开门

作为店主，如果有歇业酒馆或 LLM 配置缺失，我希望 dashboard 用明显但不恐吓的方式告诉我哪里阻塞，以及下一步去哪里修。

### 店主：已有访客互动

作为店主，我希望看到最近回访者、会话、owner-visible 访客反馈入口，用于复盘酒馆体验，而不是对访客进行公开展示或社交运营。
...
- CTA 可包含：创建酒馆、查看发现页、进入第一个需要处理的酒馆。
- Hero 文案必须强调“真实坐标、店主确认、AI 草稿辅助”。

### 2. 经营卡片

保留或增强现有 metrics：酒馆数 / 营业数；访客数 / 回访者；会话数 / 消息量；AI/LLM 配置状态（若只能前端推断，则标注为配置入口，不承诺精确健康检查）；owner-visible feedback 状态（访客 notes 入口或待处理数量，如 API 可复用）。

### 3. 下一步建议

扩展 `owner-summary` 的 next actions，使其能表达：无酒馆 → 创建第一间酒馆；有歇业酒馆 → 检查 LLM / 营业状态；有回访者或会话 → 复盘最近互动；有访客 notes 能力 → 进入具体 tavern 的 owner-visible notes 区域或提示去 tavern 管理。

### 4. 入口整合

Dashboard 应提供清晰入口，但不重复完整管理功能：`/create` 开店 / AI 草稿辅助；`/tavern/:id` 单酒馆详情、访客 notes、关系/角色治理；通知入口可复用 `NotificationBell` 或以“通知中心入口”形式展示，不要求本任务新建通知中心。

### 5. 空态 / 错误态

- 无酒馆、无访客、无会话、API 局部失败都需要有高质量空态。
...
This completed child task was rehydrated from `.trellis/tasks/archive/2026-04/04-29-owner-dashboard-presentational-mvp/` to `.trellis/tasks/04-29-04-29-owner-dashboard-presentational-mvp/` because parent task `.trellis/tasks/04-28-new-features-brainstorm/task.json` still referenced the dated child ID.

Fresh verification on current workspace:

* `npm --prefix .\frontend run typecheck` — passed.
* `npm --prefix .\frontend run build` — passed.
* `npm --prefix .\frontend test` — passed, including `owner-summary-test: ok` and `owner-dashboard-layout-test: ok`.
* Playwright command: `$env:OWNER_DASHBOARD_URL='http://127.0.0.1:5183/owner'; node .\.trellis\tmp\playwright-mainline\owner-dashboard-visual-acceptance.cjs` — passed.
...
Checked: desktop owner summary, mobile owner guide, create/discover/notification CTAs, owner-visible feedback boundary, safe LLM status, and no horizontal overflow.
