...
├── api/v1/                  # native APIRouter，目前主要集中在 taverns.py
├── contracts/               # Pydantic request/response contracts
├── application/             # TavernApplicationService 等 use case facade
...
- `backend/src/fablemap_api/application/taverns.py`：native application facade 已经偏大，后续需要拆分 bounded context。
- `backend/src/fablemap_api/api/v1/taverns.py`：native v1 endpoint 已经偏大，后续需要拆分 route modules。
...
- Backend：`api/v1/taverns.py`、`application/taverns.py`、`contracts/taverns.py`。
...
- `contracts/` 负责 request/response shape；临时 `FlexibleBody` 可保留，但新 endpoint 应逐步收敛到明确字段。
...
本设计推荐 **A（正式纳入）**，因为代码已经被提交并接入 `main.py`；否则全量 backend import 在干净环境里会因缺依赖失败。
...
目标：native route + feature 接管 product UI。
...
| `api/v1/taverns.py` / `application/taverns.py` 继续膨胀 | 新架构重复形成 monolith | Phase 2 按 bounded context 拆分 |
