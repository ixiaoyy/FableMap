...
- 仓库根目录 `index.html`：页面入口 / 操作台
- 生成后的 `bundle/index.html`：结果预览页 / 展示页
...
- 已落地：bundle HTML 预览页，可直接本地打开生成后的 `bundle/index.html`
...
- 下一步：继续把 nearby 页面入口往更轻量的位置选择 / 交互式 demo 的方向推进
...
下一阶段：继续细化 nearby 页面入口、位置选择交互与 `bundle/manifest.json` 的 Godot 对接字段。
