...
如果你现在只是想**先看一眼效果，不想跑任何命令**，可以直接打开：

- 仓库根目录 `index.html`

这是一个 repo 内已经准备好的单页面静态样品入口。
...
- 已落地：仓库根目录单页面体验入口，可直接打开 repo 内 `index.html`
- 已落地：nearby 体验入口，可直接把附近坐标导出为 `world.json` 与 bundle 预览页
- 已落地：Overpass 在线请求健壮性增强（超时 / 重试 / 错误提示）
- 已落地：本地缓存层，缓存原始 Overpass JSON
- 下一步：继续把 nearby 体验入口往更像真正单页面 / 交互式 demo 的方向推进
...
下一阶段：继续细化 nearby / bundle 预览交互与 `bundle/manifest.json` 的 Godot 对接字段。
