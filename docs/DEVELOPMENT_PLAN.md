...
2026-09-08：用户确认的技能前置 S0 已实现，统一 270 体力上限、基础工具耗能、小数保存和正常/晚睡恢复；后台检查与 Web、Windows 导出通过，见 [S0 验证记录](../.trellis/tasks/09-04-skills-recipe-unlocks-v1/verification-s0.md)。开发存档升级为封装 v2 / 状态 v14，旧档拒绝读取，不自动迁移或覆盖。完整技能成长、职业、配方与作物/鱼种映射仍待实施；差异见 [复刻差异表初版](REPLICA_COVERAGE.md)。
...
| .trellis/tasks/09-04-pre-pivot-life-sim-foundation | 第一阶段的十个先行批次，精确机制以对应任务为准；完整范围见 PHASE_PLAN |
