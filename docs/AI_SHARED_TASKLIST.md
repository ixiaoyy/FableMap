...
- `P2` · `planned` · 动态扰动解释层与状态接口对齐：把现实信号 -> 派生指标 -> UI / NPC / event 的折叠链路与 `DISTURBANCE_MODEL`、`WORLD_SCHEMA` 对齐。
...
- `W1` · `in_progress` · 地图式世界观察窗 v1：让 `bundle/page` 主视图真正以地图为主，而不是文字卡片。
- `W1a` · `planned` · 地图观察窗 v1 收尾验证与文档同步：补相关测试、README / change 文档、最小验证与回归检查，完成这一轮地图观察窗工作的正式收束。
- `W2` · `planned` · Web-2D 骨架 v0.1：定位 / 抓取 / 转义 / 绘制的第一版闭环。
- `W3` · `planned` · 地图交互层与侧边信息面板：点选、悬停、缩放、选中态与详情面板。

### B. 视觉转义 / 模式切换

- `V1` · `planned` · OSM -> 2D 建筑实体视觉转义规则库：医院、便利店、银行、公园、道路等首批规则。
- `V2` · `planned` · 现实路况 -> NPC / 拥挤隐喻动态代理层。
