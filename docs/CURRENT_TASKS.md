...
| GC-05 Tavern 模型 + service 层 | `in_progress` | TavernCharacter talkativeness / Tavern group_chat_enabled+config / tavern_group_chat_payload() |
| GC-06 群聊 API 路由 | `pending` | `/api/taverns/{id}/group-chat/*` 路由 |
| GC-07 前端群聊 API 封装 | `pending` | tavernService.js 群聊方法 |
| GC-08 TavernGroupChatRoom 界面 | `pending` | 多角色聊天 UI |
| GC-09 店主群聊配置界面 | `pending` | talkativeness 滑块、策略选择 |
| GC-10 群聊测试 | `pending` | 发言选择、LLM 调用、消息持久化测试 |
