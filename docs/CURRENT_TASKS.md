...
FableMap 已从「AI 生成游戏地图」的战略方向，转向「空间 UGC 平台」。
...
| T1.2 后端 `tavern.py` | `done` | 空间 CRUD 核心模块 |
| T1.3 `/api/taverns/*` 路由 | `done` | 空间 CRUD API 端点 |
| T1.4 前端 `tavernService.js` | `done` | 空间服务层 |
| T1.5 空间地图标记 | `done` | WorldMap.jsx 复用 POI 标记层展示空间，点击触发 activeTavernId |
| T1.6 空间创建面板 | `done` | TavernCreatePanel.jsx 已实现 |
| T1.7 Chat Panel 接入 | `done` | TavernChatRoom 已实现 |
| T1.8 空间入场 | `done` | enter_tavern API 已实现 |
...
| 任务 | 状态 | 说明 |
|------|------|------|
| T4.1 我的空间面板 | `done` | 店主控制台 MVP：空间摘要、筛选、刷新、创建/编辑/AI 配置/删除入口 |
| T4.2 LLM 配置 UI | `done` | TavernCreatePanel Step 2 接入 LLMConfigForm（24 后端）；新增 /api/llm/test-config 端点，支持创建空间前测试 LLM 连接 |
| T4.3 Token 统计面板 | `done` | 店主控制台展示总 Token、平均用量、空间用量排行；后端同步 token_used 到可展示 payload |
| T4.4 空间搜索发现 | `done` | 地图发现页展示附近空间，支持搜索、入口/状态筛选、排序与 marker/入场联动 |
| T4.5 标记分类显示 | `done` | 地图空间 marker、图例与发现列表按公开/密码/私人分类展示 |
...
| T5.4 移动端适配 | `done` | styles.css 新增 @media (max-width: 720px)，覆盖 TavernChatRoom、空间创建面板的移动端布局 |
| T5.5 性能优化 | `done` | 空间发现搜索索引缓存、地图 marker 上限与列表逐步加载，降低大量空间渲染压力 |
...
| T5.8 店主身份写操作贯通 | `done` | 前端店主控制台创建/编辑/删除空间、AI 配置和角色管理请求携带 X-User-Id，创建 payload 兜底写入 owner_id |
...
| T5.11 会话详情、导出与隐私边界 | `done` | 店主控制台可打开访客会话详情并生成/复制导出文本；聊天历史、导出、搜索和批量编辑接口限制为访客本人或空间主人访问 |
...
| T5.14 店主会话关键词搜索 | `done` | 店主控制台新增按空间搜索访客聊天记录，可从命中消息直接打开完整会话；后端搜索接口补空查询、结果上限和权限回归测试 |
...
| P1-04 世界书命中测试器 | `done` | POST /api/worldinfo/test 端点（确定性关键词/正则匹配，无 LLM）；WorldBookEditor 内置命中测试区；新增独立 WorldBookTester.jsx 组件（支持空间上下文、历史消息扫描）；styles.css 新增 .wbt-* 样式 |
| P1-05 空间包导入 / 导出 | `done` | 店主可导出不含 API Key/密码/访客聊天的空间包，并可在指定坐标导入为新空间 |
| P1-06 空间模板卡片 UI | `done` | 新增 3 个内置空间模板和模板安装页，支持搜索、标签筛选和一键挂载 |
| P1-07 输出修正 / 风格护栏 | `done` | 新增输出护栏规则引擎、店主编辑器、保存/预览 API；AI 回复落库前自动应用启用规则，无效正则不中断聊天 |
| P1-08 店主控制台重新分组 | `done` | 店主后台新增总览 / 空间 / 访客 / AI / 高级工具分组导航；高级工具台集中角色、世界书、护栏和空间包入口；抽出 OwnerConsoleSections.jsx |

### Phase 8: Prompt 与运行预设 (FM-VT P2)

| 任务 | 状态 | 说明 |
|------|------|------|
| P2-01 结构化记忆模型 | `done` | 新增 MemoryAtom 模型、空间 `_memory_atoms` 持久化、CRUD API、前端 service 封装和 private / owner / public 可见性权限边界 |
...
| P2-05 预设管理器 | `done` | 新增 runtime_presets / active_preset_id / memory_policy，店主可复制内置运行预设、编辑模型参数、捕获当前段落/护栏组合、保存并应用到空间；空间包导出包含预设但不含 API Key |
...
| UX-02 卡片化空间发现列表 | `done` | 空间发现列表已卡片化，支持搜索、筛选、排序、逐步加载和 marker 联动 |
| UX-03 移动端空间体验 | `done` | 720px 以下三栏折叠为单列，角色列表变为抽屉，输入区贴底，上下文 / 记忆面板变为底部抽屉 |
...
| 任务 | 状态 | 说明 |
|------|------|------|
| QA-01 开店向导回归测试 | `done` | 新增 `tests/test_tavern_create_wizard_regression.py`，覆盖公开 / 密码 / 私人空间、无 AI 配置、AI 测试成功 / 失败、导入角色卡和手动创建角色 |
| QA-02 记忆权限测试 | `done` | 新增 `tests/test_tavern_memory_permissions.py`，覆盖访客 private、店主 owner/public、跨访客拒绝和空间包导出隐私边界 |
...
### Phase 11: 群聊 (FM-VT GC)

> 详见 [AI_SHARED_TASKLIST.md](AI_SHARED_TASKLIST.md) 第三节 FM-VT-GC。

| 任务 | 状态 | 说明 |
|------|------|------|
| GC-01~04 群聊基础框架 | `done` | 群聊模型、发言选择策略、临时上下文注入已有实现 |
| GC-05 Tavern 模型 + service 层 | `done` | TavernCharacter talkativeness / Tavern group_chat_enabled+config / tavern_group_chat_payload() |
| GC-06 群聊 API 路由 | `done` | `/api/taverns/{id}/group-chat/*` 路由 |
| GC-07 前端群聊 API 封装 | `done` | tavernService.js 已封装群聊配置、发送、历史和 talkativeness 方法 |
| GC-08 TavernGroupChatRoom 界面 | `done` | TavernChatRoom 群聊模式接入空间级 sendGroupChat/history，多角色消息流展示头像和角色名 |
| GC-09 店主群聊配置界面 | `done` | 店主控制台群聊弹窗已支持开关、策略选择、每轮回应配置和角色 talkativeness 滑块 |
| GC-10 群聊测试 | `done` | 已修复消息顺序断言脆弱性；完整回归 216 passed |
| GC-11 群聊记忆与回访联动 | `done` | 群聊发送后返回 visitor_state / created_memories，并刷新访客记忆面板 |
...
1. 它强化了「地图浏览 → 空间发现 → 入场 → 对话 → 写回 → 回访」链路
2. 它提升了空间创建的便利性、角色配置的灵活性或对话体验的真实感
3. 它符合「平台做地图和入口，空间主人做内容」的最小平台原则

如果一个新需求满足以下特征，则应谨慎：

1. 主要目标是让地图更炫，而不是让空间体验更好
