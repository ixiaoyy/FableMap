...
在已有 GC-06 酒馆群聊路由基础上，为前端提供稳定的持久群聊 API 调用方法：

1. 读取酒馆群聊配置和角色发言积极度。
2. 保存酒馆群聊开关、策略和角色发言积极度。
3. 发送酒馆群聊消息并接收多角色回复。
4. 查询酒馆群聊历史。
...
- `tests/test_group_chat.py`（仅限酒馆级群聊路由冒烟覆盖）
...
- 修正酒馆级群聊 service 运行时归一化和 `rules` 后端发送链路，保证新增封装对应接口可冒烟。
- 更新共享任务清单和本次变更记录。
