...
GC-06 已提供 `/api/taverns/{id}/group-chat/*` 空间级群聊路由，但前端服务层还没有对应封装。后续 GC-08 / GC-09 的群聊房间和店主配置界面需要通过统一的 `tavernService.js` 方法调用这些接口，而不是在组件内手写 URL。

## 改了什么

| 文件 | 说明 |
|------|------|
| `frontend/src/services/tavernService.js` | 新增空间级持久群聊 API 方法：`getGroupChatConfig`、`updateGroupChatConfig`、`sendGroupChat`、`getGroupChatHistory`、`updateTalkativeness` |
| `fablemap/web/service.py` | 修正空间级群聊配置归一化、`rules` 本地后端发送链路和群聊历史聚合，保证前端封装对应接口可冒烟 |
| `tests/test_group_chat.py` | 增加空间级群聊路由冒烟测试，覆盖配置更新、talkativeness、规则后端发送和历史返回 |
...
- 空间级群聊发送支持 `rules` / `rule_based` / `public_welfare` 本地后端，不再尝试创建不存在的外部 client。
...
- 当前仍保留临时群聊方法和空间级群聊方法两套入口；后续 UI 收口时需要明确选择使用哪套链路。
