...
本次变更完成 Phase 4 的 `T4.3 Token 统计面板`，让店主可以在“我的酒馆控制台”中查看自己的酒馆 LLM 使用量。

## 为什么改

Phase 3 已经完成真实 LLM 调用和 token 统计记录，但店主侧缺少可见入口。T4.3 将已有 `token_used` 数据整理为控制台面板，帮助店主判断哪些酒馆正在产生模型消耗。
