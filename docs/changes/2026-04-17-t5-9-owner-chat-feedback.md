...
补齐“对话 → 写回 → 回访反馈”的店主侧闭环：店主控制台新增“最近对话会话”面板，展示自己空间中的访客会话摘要、访客昵称、角色、最后消息与消息数量。

## 改了什么

| 文件 | 变更 |
|------|------|
| `fablemap/web/router.py` | `/api/chats` 在店主查看自己空间时返回全访客会话；会话摘要增加 `visitor_name`、`tavern_name`、`last_role` |
...
- 店主请求 `/api/chats?tavern_id={id}` 且 `X-User-Id` 等于空间 `owner_id`：返回该空间所有访客会话。
