...
T2.1 原为 `partial`：创建流程已有基础角色导入和手动添加，但缺少完整字段编辑、已有酒馆角色维护，以及头像/表情立绘的持久化闭环。

## 改动

- 新增 `CharacterEditor.jsx`，统一编辑角色名称、描述、性格、场景、系统提示词、开场白、示例对话、备用开场白、标签、默认头像和标准表情立绘。
- 创建酒馆 Step 3 支持编辑已导入角色，并用同一编辑器手动新增角色。
- 店主控制台新增 `CharacterManagementModal.jsx` 角色管理入口，可对已有酒馆新增、导入、编辑、删除角色。
- 创建完成后会把已新增角色回填到前端酒馆对象，角色数量立即可见。
...
- 修复角色精灵图 API 的服务层调用，`/characters/{id}/sprites` 可以读取并保存到酒馆角色数据。
