...
### 2.11 Preset Import Preview API
...
原则：
- 该 API 只预览，不写入 `Tavern.runtime_presets`、`prompt_blocks`、`world_info`、`characters`、记忆或 StateCard。
...
### 4.7 Preset Import Preview UI（预设导入预览）

职责：
- 在店主管理面板的酒馆卡片 / 高级工具中提供“预览导入”入口。
- 允许店主粘贴预设 JSON，先在前端做 JSON 解析错误提示，再调用后端生成风险报告。
- 展示 supported / warning / blocked 分组、运行参数摘要和“不会应用、不会保存、不会覆盖”的边界说明。
...
├── taverns.json            # 所有酒馆配置（不含 api_key；含 gameplay_definitions / skill_packs）
├── taverns_keyvault.json   # 酒馆主人的 LLM API Key（加密）
├── _gameplay_sessions      # TavernStore 私有桶（逻辑上挂在 taverns 数据内，不公开导出）
├── _state_cards            # TavernStore 私有桶（pending/confirmed 连续性状态卡，不公开导出）
└── chat_history/
    ├── {tavern_id}/
