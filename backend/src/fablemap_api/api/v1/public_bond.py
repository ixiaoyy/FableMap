...
- POST /api/v1/taverns/{tavern_id}/characters/{character_id}/public-bond/apply
     → 访客申请结缘
- POST /api/v1/taverns/{tavern_id}/characters/{character_id}/public-bonds/{bond_id}/approve
     → 店主/管理员审批
- POST /api/v1/taverns/{tavern_id}/characters/{character_id}/public-bonds/{bond_id}/reject
     → 店主/管理员拒绝
- POST /api/v1/taverns/{tavern_id}/characters/{character_id}/public-bonds/{bond_id}/revoke
     → 店主/管理员撤销
- GET  /api/v1/taverns/{tavern_id}/public-bond-queue
     → 获取等待队列（店主可见）
- DELETE /api/v1/taverns/{tavern_id}/public-bond-queue/{queue_id}
     → 访客取消等待位置
...
router = APIRouter(prefix="/taverns", tags=["public-bond"])
...
@router.get("/{tavern_id}/characters/{character_id}/public-bond")
def get_visitor_bond(
    ...
    service = taverns_service(request)
    return service.get_visitor_bond(tavern_id, character_id, visitor_id, visitor_strength)


@router.post("/{tavern_id}/characters/{character_id}/public-bond/apply")
...
@router.get("/{tavern_id}/characters/{character_id}/public-bonds")
def get_public_bonds_for_character(
    ...
    """
    service = taverns_service(request)
    return service.get_public_bonds_for_character(tavern_id, character_id)
...
@router.post("/{tavern_id}/characters/{character_id}/public-bonds/{bond_id}/approve")
def approve_public_bond(
    ...
    - owner_note: 审批备注（可选）
    """
    approver_id = _check_owner_authority(request, tavern_id)
    body = request.json()
...
@router.post("/{tavern_id}/characters/{character_id}/public-bonds/{bond_id}/reject")
...
@router.post("/{tavern_id}/characters/{character_id}/public-bonds/{bond_id}/revoke")
def revoke_public_bond(
    ...
    """
    revoker_id = _check_owner_authority(request, tavern_id)
    body = request.json()
...
@router.get("/{tavern_id}/public-bond-queue")
def get_bond_queue(
    request: Request,
    tavern_id: str,
    ...
    with store._db.session_scope() as session:
        ...
        from fablemap_api.core.public_bond import NpcPublicBondQueue as DomainQueue
...
@router.delete("/{tavern_id}/public-bond-queue/{queue_id}")
def cancel_queue_entry(
    ...
    with store._db.session_scope() as session:
        model = (
            session.query(NpcPublicBondQueueModel)
            .filter_by(id=queue_id, tavern_id=tavern_id, status=QueueStatus.WAITING.value)
            .first()
        )
        if not model:
    ...
    return service.cancel_queue_entry(tavern_id, character_id, user_id)