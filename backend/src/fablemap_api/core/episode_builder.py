...
def build_episode_export(
    *,
    ...
    safe_tavern_name = _text(tavern_name, max_length=120) or safe_tavern_id
...
def _markdown(
    *,
    ...
    lines = [
        f"# {title}",
        "",
        "> FableMap 导出草稿：基于可见聊天记录与状态卡整理；不是平台自动创作的新剧情。",
        "",
        "## 导出范围",
        "",
        f"- 酒馆：{tavern_name}",
