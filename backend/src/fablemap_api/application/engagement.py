...
def get_engagement_service(request: Request) -> "EngagementService":
    tavern_service = request.app.state.taverns
    return EngagementService(
        store=EngagementStore(tavern_service.store),
        tavern_service=tavern_service,
    )


class EngagementService:
    def __init__(self, store: EngagementStore, tavern_service: Any):
        self._store = store
        self._tavern_service = tavern_service
    ...
    def _get_visible_tavern(self, tavern_id: str, user_id: str = "") -> Any:
        tavern = self._tavern_service._get_tavern_or_404(tavern_id)
    ...
    def _load_progress(self, tavern_id: str, visitor_id: str) -> Any:
        from ..core.engagement import VisitorEngagementProgress

        data = self._store.load_progress(tavern_id, visitor_id)
        return VisitorEngagementProgress.from_dict(data) if isinstance(data, dict) and data else VisitorEngagementProgress()

    def get_config(self, tavern_id: str, user_id: str) -> dict[str, Any]:
        tavern = self._get_visible_tavern(tavern_id, user_id)
    ...
    def update_config(self, tavern_id: str, owner_id: str, data: dict[str, Any]) -> dict[str, Any]:
        tavern = self._get_visible_tavern(tavern_id, owner_id)
        ...
        self._store.save_config(tavern_id, config.to_dict())
        return {"ok": True, "engagement_config": config.to_dict()}

    def get_visitor_progress(self, tavern_id: str, visitor_id: str) -> dict[str, Any]:
        visitor_id = self._require_user_id(visitor_id, "纪念币进度需要明确访客身份")
        tavern = self._get_visible_tavern(tavern_id, visitor_id)
        config = self._resolved_config(tavern)
        progress = self._load_progress(tavern_id, visitor_id)
    ...
    def claim_reward(self, tavern_id: str, visitor_id: str, session_id: str) -> dict[str, Any]:
        ...
        config = self._resolved_config(tavern)
        progress = self._load_progress(tavern_id, visitor_id)
        session = self._store.get_gameplay_session(tavern_id, safe_session_id)
        if not session:
        ...
        self._store.save_progress(tavern_id, session.visitor_id, progress.to_dict())
        return {
    ...
    def send_gift(self, tavern_id: str, visitor_id: str, character_id: str, gift_id: str) -> dict[str, Any]:
        ...
        self._store.save_progress(tavern_id, visitor_id, progress.to_dict())
        return {
            "success": result.success,
    ...
    def redeem_voucher(self, tavern_id: str, visitor_id: str) -> dict[str, Any]:
