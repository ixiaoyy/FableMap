"""
FableMap Rumor Application Service Mixin — 邻里传闻应用服务
"""
...
DEFAULT_RUMOR_TEMPLATES = [
    "听说附近新开了家「{tavern_name}」，听说那里的气氛很不错。",
    "之前有旅人提起过「{tavern_name}」，说那里的 NPC 很有人情味。",
    "你知道吗？「{tavern_name}」最近挺火的，很多人都去打卡。",
    "有个客人刚从「{tavern_name}」回来，说那里的体验很特别。",
    "说起来，「{tavern_name}」离这儿不远，有空可以去看看。",
]
...
class RumorApplicationMixin:
    ...
    def list_rumors(
        ...
        if source_tavern_id:
            rumors = store.list_by_source(source_tavern_id, limit, include_expired)
        else:
    ...
    def _generate_rumor_text(
        ...
        # 尝试使用 LLM 生成
        try:
            llm_config = self._get_runtime_llm_config(source_tavern_id="")
            if llm_config:
        ...
        return template.format(tavern_name=target_tavern_name)
