...
class StateCardApplicationMixin:
        ...
        *,
        ...
        for card in self.store.list_state_cards(tavern_id):
            ...
            if len(cards) >= safe_limit:
                break
        return {"tavern_id": tavern_id, "state_cards": cards, "count": len(cards), "filters": filters}

    def create_state_card(self, tavern_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        if not user_id:
        ...
        try:
            card = state_card_from_payload(data or {}, tavern_id=tavern_id, user_id=user_id, now=_utc_now_iso())
    ...
    def decide_state_card(
        ...
        tavern = self._get_tavern_or_404(tavern_id)
        self._ensure_visible(tavern, user_id)
        card = self.store.get_state_card(tavern_id, card_id)
        if not card:
    ...
    def preview_gm_layer(self, tavern_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        if not user_id:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        preview["tavern_name"] = tavern.name
        return preview
