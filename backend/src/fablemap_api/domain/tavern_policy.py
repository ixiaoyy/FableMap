...
def is_tavern_owner(tavern: Any, user_id: str) -> bool:
    owner_id = getattr(tavern, "owner_id", "")
    return bool(user_id and owner_id and owner_id == user_id)


def can_view_tavern(tavern: Any, user_id: str) -> bool:
    return getattr(tavern, "access", "") != "private" or is_tavern_owner(tavern, user_id)


def can_view_memory(atom: Any, tavern: Any, user_id: str) -> bool:
    if getattr(atom, "visibility", "") == "public":
        return True
    if is_tavern_owner(tavern, user_id):
        return True
    visible_to = {
