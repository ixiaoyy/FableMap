...
from ..core.tavern import VisitorState
...
class EngagementStore:
    ...
    def __init__(self, tavern_store: Any):
        self._tavern_store = tavern_store

    def get_tavern(self, tavern_id: str) -> Any:
        return self._tavern_store.get_tavern(tavern_id)

    def load_config(self, tavern_id: str) -> dict[str, Any]:
        tavern = self.get_tavern(tavern_id)
        config = getattr(tavern, "engagement_config", {}) if tavern else {}
        return deepcopy(config) if isinstance(config, dict) else {}

    def save_config(self, tavern_id: str, config: dict[str, Any]) -> None:
        tavern = self.get_tavern(tavern_id)
        if not tavern:
            return
        tavern.engagement_config = deepcopy(config) if isinstance(config, dict) else {}
        self._tavern_store.update_tavern(tavern)

    def load_progress(self, tavern_id: str, visitor_id: str) -> dict[str, Any]:
        state = self._tavern_store.get_visitor_state(tavern_id, visitor_id)
    ...
    def save_progress(self, tavern_id: str, visitor_id: str, progress: dict[str, Any]) -> None:
        state = self._tavern_store.get_visitor_state(tavern_id, visitor_id)
        if not state:
            state = VisitorState(visitor_id=visitor_id, tavern_id=tavern_id)
        ...
        self._tavern_store.update_visitor_state(tavern_id, state)
    ...
    def get_gameplay_session(self, tavern_id: str, session_id: str) -> Any | None:
        ...
        if callable(getter):
            return getter(tavern_id, session_id)
        return None
