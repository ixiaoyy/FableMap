...
或设置环境变量：
    export FABLEMAP_DATABASE_URL="mysql+pymysql://user:pass@localhost:3306/fablemap"
    python -m fablemap_api.infrastructure.migrate --output-root .fablemap-api
"""
...
from fablemap_api.infrastructure.storage import redact_database_url
...
def _state_card_payload(tavern_id: str, card_id: str, value: Any) -> dict[str, Any]:
    payload = dict(value) if isinstance(value, dict) else {}
    payload.setdefault("id", card_id)
    payload.setdefault("tavern_id", tavern_id)
    return payload
...
def migrate_taverns(db: Database, taverns_data: dict[str, dict[str, Any]]) -> int:
    ...
    for raw_tavern_id, tavern_dict in taverns_data.items():
        if not isinstance(tavern_dict, dict):
            continue
        tavern_id = str(tavern_dict.get("id") or raw_tavern_id).strip()
        if not tavern_id:
            continue
        ...
        with db.session_scope() as session:
            tavern_fields = {
                ...
                "place_type": tavern_dict.get("place_type", "tavern"),
            ...
            if isinstance(inline_llm, dict) and str(inline_llm.get("backend") or "").lower() in {"rules", "rule_based", "public_welfare"}:
                existing_llm = session.query(LLMConfigModel).filter(LLMConfigModel.tavern_id == tavern_id).first()
                llm_fields = {
                ...
                if not existing_llm:
                    session.add(LLMConfigModel(tavern_id=tavern_id, **llm_fields))

            for char_dict in _json_list(tavern_dict.get("characters")):
                if not isinstance(char_dict, dict):
                    continue
                char_id = str(char_dict.get("id") or f"char_{tavern_id}_{char_dict.get('name', 'unknown')}")
                char_fields = {
                ...
                existing_char = session.query(CharacterModel).filter_by(id=char_id, tavern_id=tavern_id).first()
                if existing_char:
                    _apply_fields(existing_char, char_fields)
                else:
                    session.add(CharacterModel(id=char_id, tavern_id=tavern_id, **char_fields))

            for index, wi_dict in enumerate(_json_list(tavern_dict.get("world_info"))):
                if not isinstance(wi_dict, dict):
                    continue
                wi_id = str(wi_dict.get("id") or f"wi_{tavern_id}_{index}")
                wi_fields = {
                ...
                existing_wi = session.query(WorldInfoModel).filter_by(id=wi_id, tavern_id=tavern_id).first()
                if existing_wi:
                    _apply_fields(existing_wi, wi_fields)
                else:
                    session.add(WorldInfoModel(id=wi_id, tavern_id=tavern_id, **wi_fields))
            ...
            if isinstance(visitors_data, dict):
                for visitor_id, visitor_dict in visitors_data.items():
                    ...
                    visitor_pk = f"visitor_{safe_visitor_id}_{tavern_id}"
                    visitor_fields = {
                    ...
                    existing_visitor = session.query(VisitorModel).filter_by(tavern_id=tavern_id, visitor_id=safe_visitor_id).first()
                    if existing_visitor:
                        _apply_fields(existing_visitor, visitor_fields)
                    else:
                        session.add(VisitorModel(id=visitor_pk, tavern_id=tavern_id, **visitor_fields))
            ...
            if isinstance(memory_atoms_data, dict):
                for atom_id, atom_dict in memory_atoms_data.items():
                    ...
                    if existing_atom:
                        _apply_fields(existing_atom, {"tavern_id": tavern_id, **atom_fields})
                    else:
                        session.add(MemoryAtomModel(id=atom_id, tavern_id=tavern_id, **atom_fields))
            ...
            if isinstance(sessions_data, dict):
                for session_id, session_dict in sessions_data.items():
                    ...
                    if existing_gameplay:
                        _apply_fields(existing_gameplay, {"tavern_id": tavern_id, **gameplay_fields})
                    else:
                        session.add(GameplaySessionModel(id=session_id, tavern_id=tavern_id, **gameplay_fields))
            ...
            if isinstance(state_cards_data, dict):
                for card_id, card_value in state_cards_data.items():
                    payload = _state_card_payload(tavern_id, str(card_id), card_value)
                    ...
                    existing_card = session.query(StateCardModel).filter_by(id=card_id, tavern_id=tavern_id).first()
                    if existing_card:
                        _apply_fields(existing_card, card_fields)
                    else:
                        session.add(StateCardModel(id=card_id, tavern_id=tavern_id, **card_fields))
...
def migrate_visitor_notes(db: Database, output_root: Path) -> int:
    ...
    migrated = 0
    with db.session_scope() as session:
        for tavern_id, notes in data.items():
            ...
            for note in notes:
                ...
                session.add(VisitorNoteModel(
                    id=note_id,
                    tavern_id=str(note.get("tavern_id") or tavern_id),
...
def run_migration(
    ...
    *,
    ...
    """
    if not mysql_url:
        raise ValueError("需要提供数据库 URL（优先 FABLEMAP_DATABASE_URL，FABLEMAP_MYSQL_URL 为旧别名）")

    output_path = Path(output_root or ".fablemap-api")
...
def main():
    ...
    if not database_url:
        logger.error("需要通过 --database-url / FABLEMAP_DATABASE_URL 提供数据库 URL（FABLEMAP_MYSQL_URL 仅为旧别名）")
        sys.exit(1)
